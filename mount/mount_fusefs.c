...
#define PROGNAME "mount_" FUSE4X_FS_TYPE
...
fuse_to_fssubtype(void **target, void *value, void *fallback)
{
    ...
    *(uint32_t *)target = fsbundle_find_fssubtype(FUSE4X_BUNDLE_PATH,
...
check_kext_status(void)
{
    ...
    result = getvfsbyname(FUSE4X_FS_TYPE, &vfc);
    if (result) { /* fuse4x is not already loaded */
        return ESRCH;
    }

    /* some version of fuse4x is already loaded; let us check it out */
...
//
//   mount_<FUSE4X_FS_TYPE> -o OPTIONS... <fdnam> <mountpoint>
...
main(int argc, char **argv)
{
    ...
    if (!fdnam) {
        errx(EX_USAGE, "missing fuse4x device file descriptor");
    }
    ...
    if ((errno == EINVAL) || (errno == ERANGE)) {
        errx(EX_USAGE,
             "invalid name (%s) for fuse4x device file descriptor", fdnam);
    }
...

    {
        ...
        if (fstat(fd, &sb) == -1) {
            err(EX_OSERR, "fstat failed for fuse4x device file descriptor");
        }
        ...
        if (strncmp(ndevbas, FUSE4X_DEVICE_BASENAME,
                    strlen(FUSE4X_DEVICE_BASENAME))) {
            errx(EX_USAGE, "mounting inappropriate device");
        }

        errno = 0;
        dindex = (int)strtol(ndevbas + strlen(FUSE4X_DEVICE_BASENAME),
                             NULL, 10);
        if ((errno == EINVAL) || (errno == ERANGE) ||
            (dindex < 0) || (dindex > FUSE4X_NDEVICES)) {
            errx(EX_USAGE, "invalid fuse4x device unit (#%d)\n", dindex);
        }
    }
    ...
    case ESRCH:
        errx(EX_UNAVAILABLE, "the fuse4x kernel extension is not loaded");
        break;

    case EINVAL:
        errx(EX_UNAVAILABLE,
             "the loaded fuse4x kernel extension has a mismatched version");
        break;

    default:
        errx(EX_UNAVAILABLE,
             "failed to query the loaded fuse4x kernel extension (%d)",
             result);
        break;
    }
    ...
    if ((strlen(statfsb.f_fstypename) == strlen(FUSE4X_FS_TYPE)) &&
        (strcmp(statfsb.f_fstypename, FUSE4X_FS_TYPE) == 0)) {
        if (!(altflags & FUSE_MOPT_ALLOW_RECURSION)) {
            errx(EX_USAGE,
                 "mount point %s is itself on a fuse4x volume", mntpath);
        }
    ...
    if (!volname) {
        if (daemon_name) {
            snprintf(args.volname, MAXPATHLEN, "fuse4x Volume %d (%s)",
                     dindex, daemon_name);
        } else {
            snprintf(args.volname, MAXPATHLEN, "fuse4x Volume %d", dindex);
        }
    } else {
    ...
    /* Finally! */
    result = mount(FUSE4X_FS_TYPE, mntpath, mntflags, (void *)&args);
...
showhelp()
{
    if (!getenv("MOUNT_FUSEFS_CALL_BY_LIB")) {
        showversion(0);
        fprintf(stderr, "\nThis program is not meant to be called directly. The fuse4x library calls it.\n");
    }
    fprintf(stderr, "\nAvailable mount options:\n");
    fprintf(stderr,
      "    -o allow_other         allow access to others besides the user who mounted\n"
      "                           the file system\n"
      "    -o allow_recursion     allow a mount point that itself resides on a fuse4x\n"
      ...
      "    -o noalerts            disable all graphical alerts (if any) in fuse4x Core\n"
...
{
    fprintf(stderr, "fuse4x mount version %s\n", FUSE4X_VERSION);
    if (doexit) {
        exit(EX_USAGE);
    }
}
